...
# set the background
background = pygame.image.load('spaceassets/bg1.jpg')

pygame.display.set_caption('Diablo')

romanBrawler = pygame.image.load('spaceassets/spaceviking.png')
bossimg = pygame.image.load('spaceassets/spaceship.png')
...
for i in range(no_of_soldiers):
    soldierimg.append(pygame.image.load('spaceassets/alien.png'))
    soldierimg.append(pygame.image.load('spaceassets/spaceinvader.png'))
    soldierimg.append(pygame.image.load('spaceassets/cybertron.png'))
    soldierimg.append(pygame.image.load('spaceassets/asteroid.png'))

    soldierX.append(random.randint(50, 460))
    soldierY.append(random.randint(30, 40))
    soldierspeedY.append(-1)

bulletimg = pygame.image.load('spaceassets/spaceray.png')
check = False
bulletX = 260
bulletY = 740
    ...
    for event in pygame.event.get():
        ...
        if next_boss == 0:
           ...
           # score=0
            ...
            if distance < 500:
                explosion = mixer.Sound('spaceassets/explosion.wav')
                explosion.play()
                bulletY = 730
                check = False
                bossX = random.randint(50, 300)
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                if check is False:
                    bulletSound = mixer.Sound('spaceassets/laser.wav')
                    bulletSound.play()
                    DEFAULT_IMAGE_SIZE = (200, 180)
                    romanBrawler = pygame.image.load('spaceassets/spaceviking2.png')
