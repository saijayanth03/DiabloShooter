...
# set the background
background = pygame.image.load('romeassets/bg1.jpg')

pygame.display.set_caption('Diablo')

romanBrawler = pygame.image.load('romeassets/Roman_Brawler1.png')
bossimg = pygame.image.load('romeassets/romanboss.png')
...
for i in range(no_of_soldiers):
    soldierimg.append(pygame.image.load('romeassets/romansoldier.png'))
    soldierimg.append(pygame.image.load('romeassets/germanictribsman.png'))
    soldierimg.append(pygame.image.load('romeassets/gaul.png'))
    soldierimg.append(pygame.image.load('romeassets/persian.png'))

    soldierX.append(random.randint(50, 460))
    soldierY.append(random.randint(30, 40))
    soldierspeedY.append(-1)

bulletimg = pygame.image.load('romeassets/Roman_Club.png')
check = False
bulletX = 260
bulletY = 740
    ...
    for event in pygame.event.get():
        ...
        if next_boss == 0:
           ...
           # score=0
            ...
            if distance < 500:
                explosion = mixer.Sound('romeassets/explosion.wav')
                explosion.play()
                bulletY = 730
                check = False
                bossX = random.randint(50, 300)
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                if check is False:
                    bulletSound = mixer.Sound('romeassets/laser.wav')
                    bulletSound.play()
                    DEFAULT_IMAGE_SIZE = (200, 180)
                    romanBrawler = pygame.image.load('romeassets/Guardian_ Brawler2.png')
